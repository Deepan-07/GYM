...
const Plans = () => {
    ...
    const [formData, setFormData] = useState({ planName: '', durationDays: '', price: '', description: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setShowModal(false);
            fetchPlans();
            setFormData({ planName: '', durationDays: '', price: '', description: '' });
        } catch(e) {
                        ...
                        {plans.map(plan => (
                            <div key={plan._id} className="card relative flex flex-col group border-primary/20 hover:border-primary/50 transition-colors">
                                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button className="text-blue-400 hover:text-blue-300 bg-gray-800 p-1.5 rounded"><Edit2 size={14}/></button>
                    ...
                    <div className="bg-card w-full max-w-md rounded-xl p-6 border border-gray-800">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                ...
                                <input name="planName" onChange={handleChange} required className="input-field" placeholder="e.g. Monthly Pro" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-sm text-gray-400 mb-1 block">Duration (Days)</label>
                                    <input name="durationDays" type="number" onChange={handleChange} required className="input-field" placeholder="e.g. 30" />
                                </div>
                                <div>
                                    <label className="text-sm text-gray-400 mb-1 block">Price (₹)</label>
                                    <input name="price" type="number" onChange={handleChange} required className="input-field" placeholder="e.g. 1500" />
                                </div>
                            </div>
                            <div>
                                <label className="text-sm text-gray-400 mb-1 block">Description</label>
                                <textarea name="description" onChange={handleChange} className="input-field h-24" placeholder="Features included in this plan..."></textarea>
                            </div>
